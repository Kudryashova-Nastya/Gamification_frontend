...
export const EditBuyModalWindow = observer(forwardRef(({data}, ref) => {
	const type = "profile_improve_color"
	...
	return (
		<ModalWindow ref={ref}>
			<svg className="close-ico" onClick={() => {MarketStore.closeModal()}}
			...
			<div className="checkmodal">
				<div className="header3">Выберите цвет для «{data.name || "Цветная карточка"}»</div>
				<div className="rows">
					{type === "profile_improve_color" ?
						<div className="blockname">

							<div className="selected-color">Выбранный цвет: <span style={{"color": sketchColor}}>{sketchColor}</span></div>
							<SketchPicker
								className="setcolor"
								color={sketchColor}
				...
				<button onClick={() => tryEdit(data)} className="button btn-large">Сохранить</button>
		</ModalWindow>
	);
}));