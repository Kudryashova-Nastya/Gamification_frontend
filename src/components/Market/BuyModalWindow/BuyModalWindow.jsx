	...
	return (
		<ModalWindow ref={ref}>
			...
			<div className="checkmodal">
				<div className="header3">{error ? "Покупка не удалась" : "Подтверждение покупки"}</div>
				<div className="rows">
					{!error ? <div className="blockname">
							<span className="balance-icon">Купить товар «{data.name}» за {data.price} </span>
							<img className="balance-icon" src={TUCOIN} alt=""/>
						</div> :
			...
			{!error &&
				<button onClick={() => tryBuy(data)} className="button btn-large">Покупаю</button>
			}
		</ModalWindow>
	);
}));